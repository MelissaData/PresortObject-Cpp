# Name:    MelissaPresortObjectWindowsCpp
# Purpose: Use the Melissa Updater to make the MelissaPresortObjectWindowsCpp code usable
...
$ProjectPath = "$CurrentPath\MelissaPresortObjectWindowsCpp"
...
function DownloadDLLs() {
  ...
  foreach ($DLL in $DLLs) {
    ...
    if ($quiet) {
      if ($DLL.FileName -eq "mdPresort.dll")
      {
...
function CheckDLLs() {
  ...
  if (!(Test-Path ("$ProjectPath\mdPresortEnums.h"))) {
    Write-Host "mdPresortEnums.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("$ProjectPath\mdPresort.h"))) {
    Write-Host "mdPresort.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("$ProjectPath\mdPresort.lib"))) {
    Write-Host "mdPresort.lib not found." 
    $FileMissing = $true
  }
  if ($FileMissing) {
...
Write-Host "`n====================== Melissa Presort Object ======================`n                     [ C++ | Windows | 64BIT ]`n"
